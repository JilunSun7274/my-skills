...
- **nanobot** — nanobot 相关
- **agentic-ai** — agent / coding agent 的调研与研究（能力面、自进化、harness 评测等）
- **ideas** — 纯粹的「灵光」：还没成形、值得养着的点子
- **todo** — 杂项、工具改进、研究待办，不属于其他分类的归这里
...
- 归类不明显或不属于任何现有分类时，归入 `todo`；若明显该单开一类，可提议新增分类。
...
4. 把去重后新增的想法**按内容归入对应分类**（`## 分类名` 下，类内按日期倒序）；不属于任何现有分类的归 `todo` 或与用户确认是否新增分类。源文件自带的分组（如 P0/P1/P2）不直接搬用，只作归类参考。
